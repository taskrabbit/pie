...
hljs.configure({useBr: true, language: ['javascript', 'json', 'html']});
...
lib.views.layout = pie.activeView.extend('layout', {
  ...
  setup: function() {
    ...
    this.on('click', '.js-nav-toggle', 'toggleNav');
    this.on('click', '.js-page', 'hideNav');
...
lib.views.page = pie.activeView.extend('page', {
  ...
  templateName: function() {
    return this.app.state.get('page') || 'getting-started';
  },

  processGists: function() {
    var gists = pie.array.from(this.qsa('code[data-gist]'));
    while(gists.length) {
      this.processGist(gists.shift());
    }
  },

  processGist: function(tag) {
    var filename = tag.getAttribute('data-gist');
    var content = this.gist.get('files.' + filename + '.content');
    var pre = pie.dom.createElement('<pre class="hljs">' + pie.string.escapeHtml(content) + '</pre>');
    tag.appendChild(pre);
...
app.router.map({
  '/' :         {subView: 'page', name: 'root', state: {page: app.PAGES[0]}},
  '/about' :    {subView: 'about', name: 'about', state: {page: 'about'}},
  '/:page' :    {subView: 'page', name: 'page'},
}, {
  view: 'layout'
});
...
app.helpers.register('gist', function(filename) {
  var gistId = app.i18n.t('gist');
  return '<code class="gist" data-gist="' + filename + '"></code>';
});
